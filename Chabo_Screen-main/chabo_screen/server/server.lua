...
RegisterCommand(config.screenshot.general.command_name, function(src, args, rawCommand)
    TriggerClientEvent("chabo_notify", "success", "Test", "test", 1000)
    if src ~= 0 then
        if not countdown[src] then
            CheckCooldown(src)
            TriggerClientEvent("chabo_screen:screenshot", src, config.screenshot.webhook.webhooks.storage)
        else
    ...
    embeds = {
        {
            ...
            footer = {
                text = os.date("%Y/%m/%d %X").. " - Made By Chabo Service",
